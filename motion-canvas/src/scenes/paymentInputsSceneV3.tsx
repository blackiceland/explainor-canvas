...
// V3 style constants (dryConditionsSceneV3 style: dark bg, transparent cards with subtle stroke).
const QUIET_STROKE = 'rgba(252,251,248,0.28)';
const QUIET_GLOW = 'rgba(252,251,248,0.10)';
// Beige card fill for code boxes (warm paper-like tone, semi-transparent).
const CARD_FILL_BEIGE = 'rgba(231, 220, 201, 0.06)';
const CARD_STROKE_BEIGE = 'rgba(231, 220, 201, 0.18)';

type Point = [number, number];

export default makeScene2D(function* (view) {
  // Apply dark gradient background (same as dryConditionsSceneV3).
  applyBackground(view);

  const S = OpenStyle;
  // Light text on dark background.
  const inkRgba = Colors.text.primary;
  const mutedRgba = Colors.text.muted;
  const borderRgba = CARD_STROKE_BEIGE;
  ...
  // Service circles: warm beige (like the Chapman Goods reference).
  const circleBeige = '#D8CFBF';
  const leftFill = circleBeige;
  const midFill = circleBeige;
  const rightFill = circleBeige;

  // Text inside beige circles should be dark.
  const whiteText = '#1A1A1A';

  // Wires visible on dark bg.
  const wireStroke = 'rgba(200,194,184,0.38)';
  const portBlue = slateBlueRgba;
  const pathFill = S.colors.blue;
  const focusPink = S.colors.accent;
  // Vivid pink underlay for riskScore highlight (more saturated for dark bg).
  const focusPinkUnderlay = 'rgba(255,120,150,0.55)';
  ...
  // Larger port dots for visibility.
  const portDotSize = OpenShapes.dots.port * 1.8;
  const packetDotSize = portDotSize * 0.82;

  // Light text on dark bg for code.
  const dtoText = Colors.text.primary;
  const dtoBlue = '#7AABFF'; // Lighter blue for dark bg.
  ...
  // Values color same as WebhookDto header.
  const dtoValueFill = '#C99A3A';
  ...
  const codeBorderStroke = CARD_STROKE_BEIGE;
  // Brighter DTO pulse: goes from subtle beige to vivid blue (like alarm highlight).
  const dtoBorderStroke = () => mixRgba(CARD_STROKE_BEIGE, '#5A9CFF', dtoPulse());
  // Thin border for V3 style.
  const codeBorderWidth = 1.5;
  ...
  const dtoBorderWidth = 2; // Slightly thicker for DTO pulse highlight visibility.
  // Minimal shadow for V3 style (transparent cards on dark bg).
  const cardShadowColor = QUIET_GLOW;
  ...
  // Slightly muted yellow for WebhookDto header (readable on dark bg).
  const solutionYellow = '#C99A3A';
