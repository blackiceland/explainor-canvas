...
export default makeScene2D(function* (view) {
    ...
    for (const element of scene.elements) {
        if (element.primitiveType !== 'DOMINO' || !element.domino)
            continue;
        ...
        view.add(<Node x={domino.x} y={domino.y}>
        {edges.map(([i, j], idx) => (<Line key={`${element.elementId}-${idx}`} stroke={domino.style.strokeColor} lineWidth={domino.style.lineWidth} lineCap="round" points={() => {
                    const currentAngle = angle();
                    const rotated = baseVertices.map(([x, y, z]) => rotateX(x, y, z, currentAngle));
        ...
        for (const tween of element.tweens) {
            if (tween.propertyPath !== 'rotationX')
                continue;
