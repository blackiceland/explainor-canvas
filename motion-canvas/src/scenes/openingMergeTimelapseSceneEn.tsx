...
// ── Передача эстафеты после первого слияния: ОТМЕНЕНА ─────────────────────
// ⚠️⚠️ ВСЕ ПАРЫ ЛЕТЯТ ПО ОБЩЕМУ РАСПИСАНИЮ. Отдельная механика для первого
// стыка пробовалась трижды и трижды дала артефакт:
//  1. стоячий призрак по краям — «слишком ранний призрак с не бесшовным
//     переходом в телесный»;
//  2. рождение уже в движении — «появляется плавно не сама строка сначала а
//     тень», «хаос какой-то»: призрак это блюр, а блюр, становящийся текстом,
//     всегда читается как «сначала тень, потом строка»;
//  3. ранний вылет с удлинённым полётом (LEAD 0.4 → travel 1.05) — строка
//     БЕЛЕЛА на пересечении: «коннектор становится белым, когда пересекается с
//     движущейся на него строкой».
// Причина третьего провала в геометрии, а не в числах. Фаза наложения
// tOver = travel − tTouch масштабируется вместе с полётом, и при travel 1.05
// две копии одного текста лежат друг на друге в полтора раза дольше, чем
// рассчитан SLAM_DIP. Копии светлые, на тёмном фоне их ink складывается — где
// глифы совпали, получается белое. Плюс строки шире промежутка до центра,
// поэтому ранняя пара накрывает ещё и уходящую предыдущую.
// ⚠️ Вся машинерия наложения (uTouch, tOver, SLAM_DIP, cool, mbScale) настроена
// на travel 0.30…0.65. Растягивать полёт ОДНОЙ паре нельзя — ломается сразу в
// нескольких местах. Если стык всё же надо уплотнить, единственный безопасный
// рычаг — УХОД предыдущей строки (дольше и тусклее), а не приход следующей:
// уход ничего не двигает и ни с чем не пересекается.
...
const FRAGS: Frag[] = [
  ...
  // Хвост терминальной скорости: два-три кадра на удар, читать здесь нечего и
  // не нужно — это фактура работающей машины. Поэтому однострочники и поэтому
  // они из того же домена: даже неразличимый текст обязан быть настоящим.
  same('tariffs.resolve(session.id)'),
  same('meterReader.poll(connector.id)'),
  same('session.touch(clock.instant())'),
  same('limits.check(owner.id)'),
  same('alerts.clear(connector.id)'),
  same('ledger.debit(session.id)'),
  same('firmware.ping(connector.id)'),
  same('queue.drain(depot.id)'),
  same('audit.append(session.id)'),
  same('cache.evict(session.id)'),
  same('telemetry.flush(session.id)'),
  same('locks.release(connector.id)'),
  same('receipts.issue(session.id)'),
];
...
// ⚠️ Машина обязана ОСТАНОВИТЬСЯ УЖЕ В ТЕМНОТЕ. Раньше последнее слияние
// приходилось на 6% яркости, но следом строка стояла неподвижно и гасла на
// виду — единственный за всю сцену кадр без движения, и он ломал уход (автор:
// «строчка в конце останавливается, эффект исчезновения ломается»).
// Теперь строб продолжается на секунду дольше, а гашение начинается позже и
// идёт дольше: к моменту, когда удары кончаются, в кадре 6% яркости, а
// последняя строка догасает на 1%. Остановки не видно вовсе.
const LAST_MERGE = 11.68;    // последнее — уже глубоко в темноте
const FADE_AT = 10.10;       // затемнение накрывает конвейер НА ХОДУ
const FADE_DUR = 2.10;       // длиннее прежнего: уход обязан быть незаметным
const T_END = 12.40;         // 0.2 с тёмного хвоста под монтажный стык

// Относительные интервалы между ударами: геометрия ×0.8 на разгоне.
// ⚠️ Дальше разгонять НЕЛЬЗЯ и не нужно. Чистая геометрия сходится: весь
// бесконечный хвост от 0.056 с знаменателем 0.8 даёт всего 0.28 — секунду им
// не заполнить. Да и незачем: 0.056·K ≈ 2.3 кадра между ударами — это уже
// предел, с которого машина не ускоряется, а просто РАБОТАЕТ на своей
// предельной скорости, пока её накрывает темнота. Плато здесь и есть смысл:
// не «выдохлась», а «не останавливается».
const REL = [
  1.0, 0.8, 0.64, 0.51, 0.41, 0.33, 0.26, 0.21, 0.17,
  0.136, 0.109, 0.087, 0.07,
  0.066, 0.062, 0.059,                       // последние кадры разгона
  0.056, 0.056, 0.056, 0.056, 0.056,         // терминальная скорость
...
//   10.74 10.83 10.91 10.99 11.07 11.14 11.22 11.30 11.37 11.45 11.53 11.60 11.68
//   (сухой щелчок строго по центру на каждом из этих моментов)
// ⚠️ K пересчитывается из LAST_MERGE и суммы REL, поэтому и удлинение хвоста, и
// новые термы подобраны так, чтобы K остался прежним (1.384): весь разгон до
// 10.65 стоит на своих местах с точностью до миллисекунд.
...
export default makeScene2D(function* (view) {
  ...
  // ⚠️ ВЫРАЖЕННОСТЬ СМАЗА = СКОРОСТЬ, а не факт полёта. Хвосты кремовые
  // (#EDE9E1) и лежат СЛОЙ ПОД кодом, а easeInCubic трогается с нуля: на старте
  // все четыре запаздывающие копии стоят ровно за оригиналом, и крем
  // просвечивает сквозь цветной код — строка белеет. Копия в это время идёт на
  // IN_OP 0.55, так что крем ещё и перевешивает. На коротких полётах это
  // мелькание в пару кадров, на длинном автор увидел: «val connector в движении
  // моргнул, стал на секунду белым».
  // Скорость easeInCubic пропорциональна u², её и берём множителем: пока копия
  // стоит, смаза нет вовсе; к удару он выходит на прежнюю величину.
  const launchTrail = (c: Copy, travel: number) => {
    const ghosts: Txt[] = [];
    const anims: ThreadGenerator[] = [];
    const k0 = mbScale(travel);
    const u = createSignal(0);
    spawn(u(1, travel, linear));
    c.lines.forEach((t, i) => {
      MB_LAGS.forEach((lag, k) => {
        const g = addLine(ghostLayer(), t, i, c.lines.length, c.tw, GHOST, 0);
        g.position.x(g.position.x() + c.x0);
        const f = createSignal(1);
        g.opacity(() => MB_OPS[k] * k0 * u() * u() * f());
        ghosts.push(g);
        anims.push(chain(
          waitFor(lag),
  ...
  function* runPair(i: number, f: Frag, travel: number): ThreadGenerator {
    const first = i === 0;
    const blur0 = first ? FIRST_BLUR : IN_BLUR;
