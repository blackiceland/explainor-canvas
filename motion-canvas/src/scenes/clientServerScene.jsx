...
export default makeScene2D(function* (view) {
    ...
    for (const element of scene.elements) {
        ...
        if (element.primitiveType === 'RECT' && element.rect) {
            ...
            view.add(<Line ref={ref} points={[
                    [line.startX, line.startY],
                    [line.endX, line.endY],
                ]} stroke={line.style.strokeColor} lineWidth={line.style.lineWidth} end={0} endArrow arrowSize={12}/>);
        }
        else if (element.primitiveType === 'TEXT' && element.text) {
            const text = element.text;
            view.add(<Txt ref={ref} text={text.text} x={text.centerX} y={text.centerY} fontSize={text.fontSize} fill={text.color} fontWeight={parseInt(text.fontWeight)} fontFamily="Inter, sans-serif"/>);
        }
        else if (element.primitiveType === 'CIRCLE' && element.circle) {
            const circle = element.circle;
            view.add(<Circle ref={ref} width={circle.radius * 2} height={circle.radius * 2} stroke={circle.style.strokeColor} fill={circle.style.fillColor} lineWidth={circle.style.lineWidth}/>);
        }
    }
    const animations = [];
    for (const element of scene.elements) {
