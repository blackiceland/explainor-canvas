...
import { all, createRef, createSignal, easeInCubic, easeOutBounce, easeOutCubic, waitFor, } from '@motion-canvas/core';
...
function rotateAroundBottomEdge(vertices, angle) {
...
export default makeScene2D(function* (view) {
    ...
    const dominoNode = createRef();
    const shadowNode = createRef();
    view.add(<>
      <Rect ref={shadowNode} x={() => {
      ...
      <Node ref={dominoNode} x={0} y={0}>
        {faces.map((face, faceIdx) => (<Node key={`face-${faceIdx}`}>
            {face.indices.map((vertexIdx, i) => {
                const nextIdx = face.indices[(i + 1) % face.indices.length];
                return (<Line key={`face-${faceIdx}-edge-${i}`} stroke={'#000000'} lineWidth={2} lineCap="round" lineJoin="round" closed={false} points={() => {
                        const angle = rotationAngle();
                        const dy = positionY();
                        const dz = positionZ();
    ...
    yield* all(rotationAngle(95, fallDuration, easeInCubic), positionY(hh + 10, fallDuration, easeInCubic), positionZ(dominoHeight * 0.8, fallDuration, easeInCubic));
    yield* all(positionY(hh, 0.15, easeOutBounce), rotationAngle(90, 0.15, easeOutCubic));
    yield* waitFor(1);
});
