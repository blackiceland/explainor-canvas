...
async function exportStillWithOverlay(
  ...
  // Draw overlay onto the rendered frame (skip with &overlay=0 for a clean shot).
  if (new URLSearchParams(location.search).get('overlay') !== '0') {
  ...
  import.meta.hot.send('motion-canvas:export', {
    ...
    mimeType: 'image/png',
    subDirectories: ['vstill', project.name],
  });
...
async function main() {
  ...
  const relPath = `output/vstill/${project.name}/${padded}.png`;
