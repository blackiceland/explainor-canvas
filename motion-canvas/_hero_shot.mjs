...
// Throwaway shot harness for heroProject (duplicationHateIntroScene + chargingHeroDemoScene).
const SCENE = process.env.SCENE || 'chargingHeroDemoScene';
const FPS = 30;
const OUT_DIR = 'C:/Users/black/IdeaProjects/explainor-canvas/motion-canvas/output/still/heroProject';
...
for (const f of frames) {
  const url = `http://127.0.0.1:5173/herostill.html?scene=${SCENE}&frame=${f}&fps=${FPS}&timeoutMs=900000&grid=off`;
  const t0 = Date.now();
  try {
