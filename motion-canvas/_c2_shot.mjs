...
const SCENE = process.env.SCENE || 'chapter2ReplaySceneEn';
const FPS = 30;
const OUT_DIR = 'C:/Users/black/IdeaProjects/explainor-canvas/motion-canvas/output/still/ch2Project';
...
for (const f of frames) {
  const url = `http://127.0.0.1:5173/ch2still.html?scene=${SCENE}&frame=${f}&fps=${FPS}&timeoutMs=300000&grid=off`;
  const t0 = Date.now();
  try {
